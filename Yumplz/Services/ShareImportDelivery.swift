...
enum ShareImportDelivery {
    ...
    /// Deep link that foregrounds yumplz; queue is drained separately on appear.
    ...
    static func makeDeepLinkNotification(for item: PendingImportItem) -> (name: Notification.Name, object: String, userInfo: [String: Any]) {
        (
            ...
            userInfo: [
                YumplzImportDeepLinkUserInfoKey.autoStart: item.autoStartImport,
                YumplzImportDeepLinkUserInfoKey.extractionMode: item.extractionMode.rawValue,
                YumplzImportDeepLinkUserInfoKey.importKind: item.kind.rawValue,
            ]
        )
    }
