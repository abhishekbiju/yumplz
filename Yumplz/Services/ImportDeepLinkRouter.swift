...
enum ImportDeepLinkRouter {
    ...
    static func route(payload: String, userInfo: [AnyHashable: Any]? = nil) -> Route {
        let autoStart = (userInfo?[YumplzImportDeepLinkUserInfoKey.autoStart] as? Bool) ?? false
        let extractionMode: ShareExtractionMode
        if let raw = userInfo?[YumplzImportDeepLinkUserInfoKey.extractionMode] as? String,
...
struct ImportPresentationRequest: Identifiable, Equatable {
    ...
    /// Opens the import sheet with no pre-filled source.
    static var newImport: ImportPresentationRequest {
        ImportPresentationRequest(
