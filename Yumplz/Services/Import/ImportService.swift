...
private let log = Logger(subsystem: "com.abhishekbiju.yumplz", category: "Import")
...
final class ImportService {
    ...
    private func fetchRecipeText(from url: URL) async throws -> String {
        ...
        request.setValue("yumplz/1.0 (iOS recipe importer)", forHTTPHeaderField: "User-Agent")
    ...
    var errorDescription: String? {
        ...
        case .needsVideoFile:
            return "Instagram captions can't be read from a link. " +
                   "In Instagram, tap ··· → Save Video, then share the video file to yumplz."
        }
    }
}
