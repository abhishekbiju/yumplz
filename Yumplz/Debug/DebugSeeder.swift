...
enum DebugSeeder {
    ...
    private static let seededKey = "com.abhishekbiju.yumplz.debugSeeded"

    @MainActor
    static func seedIfNeeded(into context: ModelContext) {
        seedIfLibraryEmpty(into: context)
    }

    /// Inserts sample recipes when the library has none (DEBUG builds only).
    @MainActor
    static func seedIfLibraryEmpty(into context: ModelContext) {
