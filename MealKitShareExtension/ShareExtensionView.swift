...
struct ShareExtensionView: View {
    ...
    let onComplete: () -> Void
    let onCancel: () -> Void
    /// Opens MealKit with the shared URL embedded in the deep link (primary open path).
    ...
    private var actionButtons: some View {
        VStack(spacing: 12) {
            ...
            Button {
                ...
                actionLabel("Open MealKit & Import", loading: isQueuing && queuedOpenApp)
            }
    ...
    private var successView: some View {
        VStack(spacing: 8) {
            ...
            if queuedOpenApp {
                Text(openFailed
                     ? "Saved — open MealKit manually to finish importing."
                     : "Opening MealKit…")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
            } else {
                Text("Open MealKit anytime to finish importing.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
            }
