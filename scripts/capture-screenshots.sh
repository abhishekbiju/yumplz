...
# Requires Xcode 16+ and an "iPhone 17" simulator (override with SIM_DEVICE).
set -euo pipefail
...
DERIVED="${DERIVED:-/tmp/mealkit-screenshot-derived}"
OUT="$ROOT/docs/screenshots"
BUNDLE="com.abhishekbiju.mealkit"

echo "Generating Xcode project..."
xcodegen generate

echo "Building MealKit for ${SIM_DEVICE}..."
xcodebuild \
  -scheme MealKit \
...
APP="$DERIVED/Build/Products/Debug-iphonesimulator/MealKit.app"
mkdir -p "$OUT"
...
capture() {
  ...
  SIMCTL_CHILD_MEALKIT_SCREENSHOT_MODE=1 \
  SIMCTL_CHILD_MEALKIT_SCREENSHOT_TAB="$tab" \
