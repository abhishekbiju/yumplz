...
final class ShareImportDeliveryTests: XCTestCase {
    ...
    func testLaunchURL_usesMealkitImportLaunchQuery() {
    ...
    func testMakeDeepLinkNotification_youTubeShort() {
        ...
        XCTAssertEqual(note.name, .mealKitImportDeepLink)
        XCTAssertEqual(note.object, item.value)
        XCTAssertEqual(note.userInfo[MealKitImportDeepLinkUserInfoKey.autoStart] as? Bool, true)
        XCTAssertEqual(
            note.userInfo[MealKitImportDeepLinkUserInfoKey.extractionMode] as? String,
