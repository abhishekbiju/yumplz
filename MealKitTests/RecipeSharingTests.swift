...
final class RecipeSharingTests: XCTestCase {
    ...
    func testPlainTextHasMealKitFooter() {
        let recipe = makeRecipe()
        let text = RecipeShareFormatter.plainText(for: recipe)
        XCTAssertTrue(text.contains("Shared from MealKit"), "Plain text must end with attribution footer")
    }

    // MARK: - Slice 5: deep link URL is valid with mealkit scheme
    ...
    func testDeepLinkURLIsValid() {
        ...
        XCTAssertEqual(url?.scheme, "mealkit", "URL scheme must be 'mealkit'")
